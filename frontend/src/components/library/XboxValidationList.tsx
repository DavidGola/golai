...
import type { XboxPreviewItem, XboxConfirmItem } from '@/api/userGames'
...
interface Props {
  items: XboxPreviewItem[]
  onImport: (items: XboxConfirmItem[]) => void
  importing: boolean
}

export default function XboxValidationList({ items, onImport, importing }: Props) {
  ...
  const handleImport = () => {
    const payload: XboxConfirmItem[] = rows
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={toggleAll} className="text-[12px] text-accent-soft hover:underline">
            ...
            <div key={item.game_id} className={`border-b border-border/60 last:border-b-0 ${disabled ? 'opacity-50' : ''}`}>
              <div className="flex items-center gap-2.5 px-3 py-2.5">
                ...
                <div className="h-[32px] w-[48px] flex-shrink-0 overflow-hidden rounded-[4px] bg-panel">
                  {item.cover_url && <img src={item.cover_url} alt="" className="h-full w-full object-cover" />}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[12px] font-medium text-primary">{item.title}</p>
                  {item.achievement_progress_pct != null && (
                    <p className="text-[12px] text-faint">{item.achievement_progress_pct}% succès</p>
                  )}
                </div>
                {disabled && (
