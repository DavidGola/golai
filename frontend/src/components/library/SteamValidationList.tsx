...
interface Props {
  ...
  onImport: (items: SteamConfirmItem[]) => void
  importing: boolean
}

export default function SteamValidationList({ items, onImport, importing }: Props) {
  ...
  const handleImport = () => {
    const payload: SteamConfirmItem[] = rows
      ...
      .map(({ r, item }) => ({
        ...
        hours_on_record: item.hours_on_record,
      }))
    onImport(payload)
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Game rows */}
      <div className="max-h-[320px] overflow-y-auto rounded-[10px] border border-border">
        {items.map((item, i) => {
          const row = rows[i]!
            ...
            <div key={item.game_id} className={`border-b border-border/60 last:border-b-0 ${disabled ? 'opacity-50' : ''}`}>
              <div className="flex items-center gap-2.5 px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {item.hours_on_record != null && item.hours_on_record > 0 && (
                    <p className="text-[12px] text-faint">{item.hours_on_record}h jouées</p>
                  )}
                </div>
